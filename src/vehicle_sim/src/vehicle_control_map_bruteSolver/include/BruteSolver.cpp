...
void BruteSolver::calcTrialPoints()
{
    ...
    // get step size
    double step_size[lb_current.size()];
...
#pragma omp critical
            {
                if (thread_best_score < score)
                {
                    ...
                    if (print_output)
                    {
                        std::cout << "New best score (" << i << "): " << score << " @ [";
...
void BruteSolver::setNTrials(std::vector<uint> n_trials_)
{
    n_trials = n_trials_;
}
...
void BruteSolver::setBounds(std::vector<double> lb_, std::vector<double> ub_)
{
    lb = lb_;
    ub = ub_;
    lb_current = lb_;
